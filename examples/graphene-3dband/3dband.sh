#Add your pbs script head here

VASPMATE_DIR='/home/moss/mumu/VASPMATE_V2.0.0_Oct08_2024/bin' #The path for vaspmate eg. /usr/bin/
VASP_RUN='mpirun -np 4 vasp'		#mpirun -np 4 vasp541

for SYS_name in C
do
cp ./structure/${SYS_name}.vasp ./INPOS

#>>> create POSCAR file
cp INPOS POSCAR

#--------------------------Relaxtion--------------------------
#>>> create INCAR file using VASPMATE and the file incar_rlx and incar_stc are created
${VASPMATE_DIR}/VASPMATE --i rlx
#>>> HSE calculation
#${VASPMATE_DIR}/VASPMATE --i hse stc
cp incar_rlx INCAR
#>>> correct parameters
#${VASPMATE_DIR}/VASPMATE --i_replace ISPIN 2
#${VASPMATE_DIR}/VASPMATE --i_replace ENCUT 400
#>>> create KPOINTS file
${VASPMATE_DIR}/VASPMATE --ka -par 4000 G
cp NEWKPT KPOINTS
#>>> create POSAR file
cp INPOS POSCAR
#>>> create POTCAR file
${VASPMATE_DIR}/VASPMATE --pot -PBE
cp NEWPOT POTCAR
eval $VASP_RUN >> log1.vasp 
#--------------------------------------------------------------

#------------------option(self-consistent)---------------------
${VASPMATE_DIR}/VASPMATE --i stc
cp CONTCAR INPOS
cp incar_stc INCAR
${VASPMATE_DIR}/VASPMATE --i_replace LCHARG T
#${VASPMATE_DIR}/VASPMATE --i_replace ISPIN 2
#${VASPMATE_DIR}/VASPMATE --i_replace ICHARG 2
${VASPMATE_DIR}/VASPMATE --3dkm 60 60 1
#${VASPMATE_DIR}/VASPMATE --3dkv 0.05
#${VASPMATE_DIR}/VASPMATE --3dka 8000
cp 3DbandKPT KPOINTS
cp INPOS POSCAR
eval $VASP_RUN >> log2.vasp 
#--------------------------------------------------------------

${VASPMATE_DIR}/VASPMATE --3dbs
#${VASPMATE_DIR}/VASPMATE --3dbs -b 5 6
done
#rm POSCAR KPOINTS* INCAR* POTCAR
#rm CHG* CONTCAR DOSCAR OSZICAR OUTCAR EIGENVAL PCDAT WAVECAR XDATCAR IBZKPT vasprun.xml
#END
