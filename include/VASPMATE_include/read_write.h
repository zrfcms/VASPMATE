...
typedef struct POSCAR
{
	...
	int    ifix;                    // Select 0 or normal 1
	int    iflg;                    // Direct 0 or Cartes 1
	int    nant[2];                 // number of atoms and types  //(Fe2O3) printf: 5  2
	int    typenum[MAX_NELEM];      // number of atoms of each type //(Fe2O3) printf: 2  3
	int    elemnum[MAX_NELEM];      // atomic number  //(Fe2O3) printf: 26/8
	char   elemsym[MAX_NELEM][3];   // atomic symbol  //(Fe2O3) printf: Fe/O
	char   elemeach[MAX_NELEM][3];   // atomic symbol of each atom//(Fe2O3) printf: Fe/Fe/O/O/O
...
int savposcar_bader(FILE* fp, POSCAR poscar);

int readposcar(FILE* fp, POSCAR& poscar);

int readxdatcar(FILE* fp, POSCAR& poscar); //only read line 1-7
