...
void CHGCAR::operator_bader(int argc, char* argv[])
{
	...
	if (!strcmp(argv[2], "-comb"))
	{
		...
		if (argc == 3)
		{
			chg1.readchgcar("AECCAR0");
			chg2.readchgcar("AECCAR2");		
		}
		else
		{
		...
		if (chg1.pos.nant[0] != chg2.pos.nant[0])
			printf("Atoms not same in two files!\n");
		...
		n_chg.writechgcar("CHGCAR_sum", 0);
	}
	if (!strcmp(argv[2], "-d") || !strcmp("-derive", argv[2]) || !strcmp("-calc", argv[2]))
	{
		CHGCAR chg;
		chg.readchgcar("CONTCAR");
		chg.read_file_acf("ACF.dat");
		chg.read_pot_zval("POTCAR");
		//write Bader_Charge
		FILE* fp_wri = fopen("Bader_Charge", "w");
		fprintf(fp_wri, "#Calculate the net charge, generated by VASPMATE.\n\n");
		fprintf(fp_wri, "#No.  Element      X             Y             Z        Bader_Charge      ZVAL      Net_Charge\n");
		fprintf(fp_wri, "#----------------------------------------------------------------------------------------------\n");
		for (int i = 0; i < chg.pos.nant[0]; i++)
		{
			double zval_loc = 0;
			auto it = chg.pot_zval.find(chg.pos.elemeach[i]);
			if (it != chg.pot_zval.end())
				zval_loc = it -> second;
			else
			{
				printf("Error! The element %s in CONTCAR are inconsistent with those in POTCAR.\n", chg.pos.elemeach[i]);
				fprintf(fp_wri, "# Error: The element %s in CONTCAR are inconsistent with those in POTCAR.\n", chg.pos.elemeach[i]);
				fclose(fp_wri);
				return;
			}
			fprintf(fp_wri, " %2d     %2s     %9lf     %9lf     %9lf     %9lf     %9lf    %9lf\n", i+1 , chg.pos.elemeach[i], chg.acf_x[i], chg.acf_y[i], 
							chg.acf_z[i], chg.acf_chg[i], zval_loc, zval_loc-chg.acf_chg[i]);	
		}
		fprintf(fp_wri, "#----------------------------------------------------------------------------------------------\n");
		printf("Written Bader_Charge file!\n");
		fclose(fp_wri);
	}
}
...
void CHGCAR::series_oper(int argc, char* argv[], bool oper)
{
	...
	if (oper)
	{
		...
		printf("Written CHGSUMM.vasp file!\n");
	}
	else
	{
		chg.writechgcar("CHGDIFF.vasp", 0);
		printf("Written CHGDIFF.vasp file!\n");
	}
}
...
void CHGCAR::read_file_acf(const char file[])
{
	FILE* fp_acf = fopen(file, "r");
	if (fp_acf == NULL)
	{
		printf("%s is not exist!\n", file);
		fclose(fp_acf);
	...
	char buf[1024];
	bool first_line = false; //begin with fist "------------"
	while (fgets(buf, 1024, fp_acf) != NULL)
	{
		int index;
        double x, y, z, charge;
		if (strstr(buf, "------------") != NULL && !first_line)
		{
			first_line = true;
			continue;
		}
		else if (strstr(buf, "------------") != NULL && first_line)
		{
			break;
		}
		if (first_line)
		{
			sscanf(buf, "%d %lf %lf %lf %lf", &index, &x, &y, &z, &charge);
			acf_chg.push_back(charge);
			acf_x.push_back(x);
			acf_y.push_back(y);
			acf_z.push_back(z);
		}
...
void CHGCAR::read_pot_zval(const char file[])
{
	FILE* fp_pot = fopen(file, "r");
	if (fp_pot == NULL)
	{
		printf("%s is not exist!\n", file);
		fclose(fp_pot);
	...
	vector<string> potzval;
	while (fgets(buf, 1024, fp_pot) != NULL)
	{
		...
		if (strstr(buf, "ZVAL") != NULL) //   POMASS =   10.811; ZVAL   =    3.000    mass and valenz
		{
        	string zval;
        	int is_pom = 0;
			for (int i = 0; i < strlen(buf); i++) 
			{
				...
				if (is_pom)
				{
					if (isdigit(buf[i]) || buf[i] == '.') 
						zval.push_back(buf[i]);
				}
			}
			potzval.push_back(zval);
		}
	}
	if(potelem.size() != potzval.size())
	{
		printf("The number of elements in POTCAR does not match the corresponding number of ZVALs.\n");
		return;
	}
	else
	{
		for(int i = 0; i < potelem.size(); i++)
			this->pot_zval.insert({potelem[i] , stod(potzval[i]) });
	}
	fclose(fp_pot);
}
