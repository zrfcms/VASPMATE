...
void neb_Points(const char name1[50], const char name2[50], int N)
{
	...
	system(str);
	sprintf(str, "cp %s %s", "INIOUT", "00/OUTCAR");
	system(str);
	char tmp1[50];
	sprintf(tmp1, "%s/OUTCAR", name);
	sprintf(str, "cp %s %s", "FINOUT", tmp1);
	system(str);
	char tmp2[50];
...
void neb_idpp(const char name1[50], const char name2[50], int N)
{
	/*int argc_idpp = 10;
	char** argv_idpp=(char**)malloc(argc_idpp*sizeof(char*));
	for(int i = 0; i < argc_idpp; i++)
        argv_idpp[i] = (char*)malloc(50 * sizeof(char));
	strcpy(argv_idpp[0],"");
	strcpy(argv_idpp[1],"INIPOS");
	strcpy(argv_idpp[2],"FINPOS");
	strcpy(argv_idpp[3],"POSCAR");
	//sprintf(argv_idpp[3], "%d", N); //The number of inserted values
	sprintf(argv_idpp[5], "%d\0", 1); //1 to choose IDPP interpolation method; 2 to choose linear interpolation method
	sprintf(argv_idpp[4], "%d\0", N); //The number of inserted values
	sprintf(argv_idpp[6], "%d\0", 1000); //max_step
	sprintf(argv_idpp[7], "%d\0", 50); //max_try
	sprintf(argv_idpp[8], "%lf\0", 10e-4); //min_delta
	sprintf(argv_idpp[9], "%f\0", 0.01); //step_length
	MEP_main(argc_idpp, argv_idpp);
    for(int i = 0; i < argc_idpp; i++)
        free(argv_idpp[i]);
	free(argv_idpp);*/
	printf("This feature has suddenly encountered a serious bug. Please stay tuned for subsequent version updates.\n");
}
...
void neb(int argc, char* argv[])
{
	if (!strcmp(argv[2], "-sim"))
	{
		...
		if (argc >= 5)
			...
			d = neb_similarity("INIPOS", "FINPOS");
		printf("The difference between INIPOS and FINPOS is %lf\n", d);
		double dis = d / 0.8;
		int N = (int)(dis + 0.5);
	...
	{
		int numb = 0; int line = 0; int idpp = 0;
		int check_numb = 0; int check_line = 0; int check_idpp = 0;
		check_numb = check_number(argc, argv, &numb);
		check_line = check_para(argc, argv, "-line", 0, &line);
		check_idpp = check_para(argc, argv, "-idpp", 0, &idpp);
		if (check_numb == 2)
		{
			double d = neb_similarity("INIPOS", "FINPOS");
			if (d <= 5)
			{
				double dis = d / 0.8;
				int N = (int)(dis + 0.5);
				printf("The suggest number of points %d\n", N);
				neb_Points("INIPOS", "FINPOS", N);
			}
			else
			{
		...
		else if (check_numb == 1)
		{
			if (atoi(argv[numb]) < 1)
				printf("The number is too small!\n");
			else
			{
				if ((check_line == 2 && check_idpp ==  2)||(check_line == 1))
					neb_Points("INIPOS", "FINPOS", atoi(argv[numb]));
				else if ((check_line == 2 && check_idpp ==  1))
				{
					neb_idpp("INIPOS", "FINPOS", atoi(argv[numb]));
				}
			}
		}
	}
