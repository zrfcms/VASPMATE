...
void DOS::Doscsv(vector<string> element, int typenum[], char elemsym[][3], int nant[], int max_element, int ISPIN, int LORBIT)
{
	if (ISPIN == 1 && LORBIT == 10)
	{
		FILE* fp = fopen("PDOS_SUM.csv", "w");
		FILE* _fp = fopen("IPDOS_SUM.csv", "w");
		vector<double> IDOS = Intergral(this->energy, this->dos_nospin);
		if (max_element <= 57)
		{
			fprintf(fp, "Energy,Total,");
			fprintf(_fp, "Energy,Total,");
			for (int i = 0; i < element.size(); i++)
			{
				fprintf(fp, "%s-s,%s-p,%s-d", element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(fp, ",");
				fprintf(_fp, "%s-s,%s-p,%s-d", element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(_fp, ",");
			}
			fprintf(fp,"\n");
			...
			{
				int cnt = 0;
				fprintf(fp, "%lf,%lf,", this->energy_ion[0][k] - this->efermi, this->dos_nospin[k]);
				fprintf(_fp, "%lf,%lf,", this->energy_ion[0][k] - this->efermi, IDOS[k]);
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 3; m++)
					{
						double temp_dos = 0;
						double temp_idos = 0;
						for (int j = 0; j < typenum[e]; j++)
						{
							temp_dos += this->orbit_dos[m][j + cnt][k];
							temp_idos += this->Idos[m][j + cnt][k];
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 2 || e < nant[1] - 1) fprintf(fp, ",");
						fprintf(_fp, "%lf", temp_idos);
						if (m < 2 || e < nant[1] - 1) fprintf(_fp, ",");
					}
					cnt = cnt + typenum[e];
				}
				fprintf(fp,"\n");
		...
		else if (max_element > 57)
		{
			...
			for (int i = 0; i < element.size(); i++)
			{
				fprintf(fp, "%s-s,%s-p,%s-d,%s-f", element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(fp, ",");
				fprintf(_fp, "%s-s,%s-p,%s-d,%s-f", element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(_fp, ",");
			}
			fprintf(fp,"\n");
			...
			for (int k = 0; k < this->NEDOS; k++)
			{
				...
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 4; m++)
					{
						...
						fprintf(fp, "%lf", temp_dos);
						if (m < 3 || e < nant[1] - 1) fprintf(fp, ",");
						fprintf(_fp, "%lf", temp_idos);
						if (m < 3 || e < nant[1] - 1) fprintf(_fp, ",");
					}
					cnt = cnt + typenum[e];
				}
				fprintf(fp,"\n");
				...
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 3; m++)
					{
						...
						if (m == 0)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[4][j + cnt][k];
								temp_idos = temp_idos + this->Idos[4][j + cnt][k];
							}							
						}
						else if (m == 1)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[5][j + cnt][k] + this->orbit_dos[6][j + cnt][k] + this->orbit_dos[7][j + cnt][k];
								temp_idos = temp_idos + this->Idos[5][j + cnt][k] + this->Idos[6][j + cnt][k] + this->Idos[7][j + cnt][k];
							}							
						}
						else if (m == 2)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[8][j + cnt][k] + this->orbit_dos[9][j + cnt][k] + this->orbit_dos[10][j + cnt][k]
													+ this->orbit_dos[11][j + cnt][k] + this->orbit_dos[12][j + cnt][k];
								temp_idos = temp_idos + this->Idos[8][j + cnt][k] + this->Idos[9][j + cnt][k] + this->Idos[10][j + cnt][k]
													  + this->Idos[11][j + cnt][k] + this->Idos[12][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 2 || e < nant[1] - 1) fprintf(fp, ",");
				...
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 4; m++)
					{
						...
						else if (m == 3)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[13][j + cnt][k] + this->orbit_dos[14][j + cnt][k] + this->orbit_dos[15][j + cnt][k]
													+ this->orbit_dos[16][j + cnt][k] + this->orbit_dos[17][j + cnt][k] + this->orbit_dos[18][j + cnt][k]
													+ this->orbit_dos[19][j + cnt][k];
								temp_idos = temp_idos + this->Idos[13][j + cnt][k] + this->Idos[14][j + cnt][k] + this->Idos[15][j + cnt][k]
													  + this->Idos[16][j + cnt][k] + this->Idos[17][j + cnt][k] + this->Idos[18][j + cnt][k]
													  + this->Idos[19][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 3 || e < nant[1] - 1) fprintf(fp, ",");
	...
	else if (ISPIN == 2 && LORBIT == 10)
	{
		...
		if (max_element <= 57)
		{
			fprintf(fp, "Energy,Total-up,Total-dw,");
			fprintf(_fp, "Energy,Total-up,Total-dw,");
			for (int i = 0; i < element.size(); i++)
			{
				fprintf(fp, "%s-s-up,%s-p-up,%s-d-up,%s-s-dw,%s-p-dw,%s-d-dw", element[i].c_str(),element[i].c_str(),element[i].c_str(), element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(fp, ",");
				fprintf(_fp, "%s-s-up,%s-p-up,%s-d-up,%s-s-dw,%s-p-dw,%s-d-dw", element[i].c_str(),element[i].c_str(),element[i].c_str(), element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(_fp, ",");
			}
			fprintf(fp,"\n");
			...
			{
				int cnt = 0;
				fprintf(fp, "%lf,%lf,%lf,", this->energy_ion[0][k] - this->efermi, this->dos_up[k], this->dos_dw[k] * -1);
				fprintf(_fp, "%lf,%lf,%lf,", this->energy_ion[0][k] - this->efermi, IDOS_up[k], IDOS_dw[k] * -1);
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 3; m++)
					{
						...
						for (int j = 0; j < typenum[e]; j++)
						{
							temp_dos += this->orbit_dos[m + 20][j + cnt][k];
							temp_idos += this->Idos[m + 20][j + cnt][k];
						}
						fprintf(fp, "%lf,", temp_dos);
						fprintf(_fp, "%lf,", temp_idos);
					}
					for (int m = 0; m < 3; m++)
					{
						...
						for (int j = 0; j < typenum[e]; j++)
						{
							temp_dos -= this->orbit_dos[m + 24][j + cnt][k];
							temp_idos -= this->Idos[m + 24][j + cnt][k];
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 2 || e < nant[1] - 1) fprintf(fp, ",");
						fprintf(_fp, "%lf", temp_idos);
		...
		else if (max_element > 57)
		{
			...
			for (int i = 0; i < element.size(); i++)
			{
				fprintf(fp, "%s-s-up,%s-p-up,%s-d-up,%s-f-up,%s-s-dw,%s-p-dw,%s-d-dw,%s-f-dw", element[i].c_str(),element[i].c_str(),element[i].c_str(), element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(fp, ",");
				fprintf(_fp, "%s-s-up,%s-p-up,%s-d-up,%s-f-up,%s-s-dw,%s-p-dw,%s-d-dw,%s-f-dw", element[i].c_str(),element[i].c_str(),element[i].c_str(), element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str(),element[i].c_str());
				if (i < element.size() - 1)
                	fprintf(_fp, ",");
			}
			fprintf(fp,"\n");
				...
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 3; m++)
					{
						...
						if (m == 0)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[28][j + cnt][k];
								temp_idos = temp_idos + this->Idos[28][j + cnt][k];
							}							
						}
						else if (m == 1)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[29][j + cnt][k] + this->orbit_dos[30][j + cnt][k] + this->orbit_dos[31][j + cnt][k];
								temp_idos = temp_idos + this->Idos[29][j + cnt][k] + this->Idos[30][j + cnt][k] + this->Idos[31][j + cnt][k];
							}							
						}
						else if (m == 2)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[32][j + cnt][k] + this->orbit_dos[33][j + cnt][k] + this->orbit_dos[34][j + cnt][k]
													+ this->orbit_dos[35][j + cnt][k] + this->orbit_dos[36][j + cnt][k];
								temp_idos = temp_idos + this->Idos[32][j + cnt][k] + this->Idos[33][j + cnt][k] + this->Idos[34][j + cnt][k]
													  + this->Idos[35][j + cnt][k] + this->Idos[36][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf,", temp_dos);
						fprintf(_fp, "%lf,", temp_idos);
					...
					for (int m = 0; m < 3; m++)
					{
						...
						if (m == 0)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos - this->orbit_dos[44][j + cnt][k];
								temp_idos = temp_idos - this->Idos[44][j + cnt][k];
							}							
						}
						else if (m == 1)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos - this->orbit_dos[45][j + cnt][k] - this->orbit_dos[46][j + cnt][k] - this->orbit_dos[47][j + cnt][k];
								temp_idos = temp_idos - this->Idos[45][j + cnt][k] - this->Idos[46][j + cnt][k] - this->Idos[47][j + cnt][k];
							}							
						}
						else if (m == 2)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos - this->orbit_dos[48][j + cnt][k] - this->orbit_dos[49][j + cnt][k] - this->orbit_dos[50][j + cnt][k]
													- this->orbit_dos[51][j + cnt][k] - this->orbit_dos[52][j + cnt][k];
								temp_idos = temp_idos - this->Idos[48][j + cnt][k] - this->Idos[49][j + cnt][k] - this->Idos[50][j + cnt][k]
													  - this->Idos[51][j + cnt][k] - this->Idos[52][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 2 || e < nant[1] - 1) fprintf(fp, ",");
				...
				for (int e = 0; e < nant[1]; e++)
				{
					for (int m = 0; m < 4; m++)
					{
						...
						else if (m == 3)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos + this->orbit_dos[37][j + cnt][k] + this->orbit_dos[38][j + cnt][k] + this->orbit_dos[39][j + cnt][k]
													+ this->orbit_dos[40][j + cnt][k] + this->orbit_dos[41][j + cnt][k] + this->orbit_dos[42][j + cnt][k]
													+ this->orbit_dos[43][j + cnt][k];
								temp_idos = temp_idos + this->Idos[37][j + cnt][k] + this->Idos[38][j + cnt][k] + this->Idos[39][j + cnt][k]
													  + this->Idos[40][j + cnt][k] + this->Idos[41][j + cnt][k] + this->Idos[42][j + cnt][k]
													  + this->Idos[43][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf,", temp_dos);
						fprintf(_fp, "%lf,", temp_idos);
					...
					for (int m = 0; m < 4; m++)
					{
						...
						else if (m == 3)
						{
							for (int j = 0; j < typenum[e]; j++)
							{
								temp_dos = temp_dos - this->orbit_dos[53][j + cnt][k] - this->orbit_dos[54][j + cnt][k] - this->orbit_dos[55][j + cnt][k]
													- this->orbit_dos[56][j + cnt][k] - this->orbit_dos[57][j + cnt][k] - this->orbit_dos[58][j + cnt][k]
													- this->orbit_dos[59][j + cnt][k];
								temp_idos = temp_idos - this->Idos[53][j + cnt][k] - this->Idos[54][j + cnt][k] - this->Idos[55][j + cnt][k]
													  - this->Idos[56][j + cnt][k] - this->Idos[57][j + cnt][k] - this->Idos[58][j + cnt][k]
													  - this->Idos[59][j + cnt][k];
							}							
						}
						fprintf(fp, "%lf", temp_dos);
						if (m < 3 || e < nant[1] - 1) fprintf(fp, ",");
	...
	printf("Written PDOS_SUM.csv File!\n");
	printf("Written IPDOS_SUM.csv File!\n");
}
...
void DOS::GetDos(int argc, char* argv[])
{
	...
	if (!strcmp(argv[2], "-t") || !strcmp(argv[2], "-T")) //total VASPMATE --dos -t/-T
	{
		...
		elem.resize(nant[1]);
		for (int i = 0; i < nant[1]; i++)
			elem[i] = string(elemsym[i]);
		dos.Doscsv(elem, typenum, elemsym, nant, max_element, atoi(ISPIN.c_str()), atoi(LORBIT.c_str()));
	}
	...
	{
		vector<int> num = TranArgvToAtomIndex(argc, argv, nant, typenum, elemsym, xyz, "w");
		string postfix;
		for (int i = 3; i < argc; i++)
		{
			if (i != argc - 1)
