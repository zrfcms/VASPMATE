	...
	, vector<vector<vector<vector<double> > >> ion_dos_up, vector<vector<vector<vector<double> > >> ion_dos_dw, int argc, char* argv[])
{
	...
	if (argc > 2 && !strcmp("-ib", argv[2])) //select band
	{
	...
	if (argc > 2 && !strcmp("-io", argv[2])) //select orbit
		start_orbit_argv = 3;
...
void EIGENVAL::get3dband(vector<int> band_index, double efermi)
{
	...
	double minx = -(rec[0][0] + rec[1][0]) / 2;
	double miny = -(rec[0][1] + rec[1][1]) / 2;
	for (int i = 0; i < kpos.size(); i++)
	{
		fprintf(fpkx, " %lf\n", kpos_cartes[i][0]);
		fprintf(fpky, " %lf\n", kpos_cartes[i][1]);
	...
	if (band_index.empty())
	{
		if (ISPIN == 1)
		{
			...
			printf("homo_index: %d	lumo_index: %d\n", *band_index_up.begin(), *band_index_up.begin() + 1);
			vector<char*> filename = { "BAND_HOMO.grd","BAND_LUMO.grd" };
			vector<char*> spafilename = { "BAND_HOMO.spa","BAND_LUMO.spa" };
			for (int i = 0; i < 2; i++)
			{
				FILE* fp = fopen(filename[i], "w");
				double minz = eigenvalue[0][0][ho_lu[i] - 1] - efermi;
				double maxz = eigenvalue[0][0][ho_lu[i] - 1] - efermi;
				for (int j = 0; j < kpos.size(); j++)
				{
					minz = min(minz, eigenvalue[0][j][ho_lu[i] - 1] - efermi);
					maxz = max(maxz, eigenvalue[0][j][ho_lu[i] - 1] - efermi);
					fprintf(fp, "    %5lf\n", eigenvalue[0][j][ho_lu[i] - 1] - efermi);
				}
				fclose(fp);
				printf("Written %s file!\n", filename[i]);
				FILE* fpspa = fopen(spafilename[i], "w");
				fprintf(fpspa, "ORIGIN:	%lf	%lf\n", minx, miny);
				fprintf(fpspa, "VECTOR1:	%lf	%lf\n", rec[0][0], rec[0][1]);
				fprintf(fpspa, "VECTOR2:	%lf	%lf\n", rec[1][0], rec[1][1]);
				fprintf(fpspa, "HEIGHT:		%lf	%lf\n", minz, maxz);
				fprintf(fpspa, "SCALE:		1	1	1\n");
				fprintf(fpspa, "POINT:	%d\n", kpos.size());
				for (int j = 0; j < kpos.size(); j++)
					fprintf(fpspa, " %lf	%lf	%lf\n", kpos_cartes[j][0], kpos_cartes[j][1], eigenvalue[0][j][ho_lu[i] - 1] - efermi);
				fclose(fpspa);
				printf("Written %s file!\n", spafilename[i]);
			}
		}
		else
		{
			...
			vector<vector<char*> > filename = { { "BAND_HOMO_UP.grd","BAND_LUMO_UP.grd" } ,{ "BAND_HOMO_DW.grd","BAND_LUMO_DW.grd" } };
			vector<vector<char*> > spafilename = { { "BAND_HOMO_UP.spa","BAND_LUMO_UP.spa" } ,{ "BAND_HOMO_DW.spa","BAND_LUMO_DW.spa" } };
			for (int k = 0; k < ISPIN; k++)
			{
				for (int i = 0; i < 2; i++)
				{
					FILE* fp = fopen(filename[k][i], "w");
					double minz = eigenvalue[k][0][ho_lu[k][i] - 1] - efermi;
					double maxz = eigenvalue[k][0][ho_lu[k][i] - 1] - efermi;
					for (int j = 0; j < kpos.size(); j++)
					{
						minz = min(minz, eigenvalue[k][j][ho_lu[k][i] - 1] - efermi);
						maxz = max(maxz, eigenvalue[k][j][ho_lu[k][i] - 1] - efermi);
						fprintf(fp, "    %5lf\n", eigenvalue[k][j][ho_lu[k][i] - 1] - efermi);
					}
					fclose(fp);
					printf("Written %s file!\n", filename[k][i]);
					FILE* fpspa = fopen(spafilename[k][i], "w");
					fprintf(fpspa, "ORIGIN:	%lf	%lf\n", minx, miny);
					fprintf(fpspa, "VECTOR1:	%lf	%lf\n", rec[0][0], rec[0][1]);
					fprintf(fpspa, "VECTOR2:	%lf	%lf\n", rec[1][0], rec[1][1]);
					fprintf(fpspa, "HEIGHT:		%lf	%lf\n", minz, maxz);
					...
					fprintf(fpspa, "POINT:	%d\n", kpos.size());
					for (int j = 0; j < kpos.size(); j++)
						fprintf(fpspa, " %lf	%lf	%lf\n", kpos_cartes[j][0], kpos_cartes[j][1], eigenvalue[k][j][ho_lu[k][i] - 1] - efermi);
					fclose(fpspa);
					printf("Written %s file!\n", spafilename[k][i]);
				}
			}
	...
	else
	{
		for (int i = 0; i < band_index.size(); i++)
		{
			for (int k = 0; k < ISPIN; k++)
			{
				...
				vector<string> spa_file(ISPIN, "BAND_B");
				if (ISPIN == 1)
				{
					tot_file[0] += to_string(band_index[i]) + ".grd";
					spa_file[0] += to_string(band_index[i]) + ".spa";
				}
				else
				{
					tot_file[0] += to_string(band_index[i]) + "_UP.grd";
					tot_file[1] += to_string(band_index[i]) + "_DW.grd";
					spa_file[0] += to_string(band_index[i]) + "_UP.spa";
					spa_file[1] += to_string(band_index[i]) + "_DW.spa";
				}
				double minz = eigenvalue[k][0][band_index[i] - 1] - efermi;
				double maxz = eigenvalue[k][0][band_index[i] - 1] - efermi;
				FILE* fp = fopen(tot_file[k].c_str(), "w");
				for (int j = 0; j < kpos.size(); j++)
				{
					minz = min(minz, eigenvalue[k][j][band_index[i] - 1] - efermi);
					maxz = max(maxz, eigenvalue[k][j][band_index[i] - 1] - efermi);
					fprintf(fp, "    %5lf\n", eigenvalue[k][j][band_index[i] - 1] - efermi);
				}
				fclose(fp);
				printf("Written %s file!\n", tot_file[k].c_str());
				FILE* fpspa = fopen(spa_file[k].c_str(), "w");
				fprintf(fpspa, "ORIGIN:	%lf	%lf\n", minx, miny);
				fprintf(fpspa, "VECTOR1:	%lf	%lf\n", rec[0][0], rec[0][1]);
				fprintf(fpspa, "VECTOR2:	%lf	%lf\n", rec[1][0], rec[1][1]);
				fprintf(fpspa, "HEIGHT:		%lf	%lf\n", minz, maxz);
				...
				fprintf(fpspa, "POINT:	%d\n", kpos.size());
				for (int j = 0; j < kpos.size(); j++)
					fprintf(fpspa, " %lf	%lf	%lf\n", kpos_cartes[j][0], kpos_cartes[j][1], eigenvalue[k][j][band_index[i] - 1] - efermi);
				fclose(fpspa);
				printf("Written %s file!\n", spa_file[k].c_str());
			}
		}
