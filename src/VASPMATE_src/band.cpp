...
vector<double> BAND::getkpointsline(double vec[3][3])
{
	...
	while (fgets(buf, 1024, fp) != NULL)
	{
		...
		if (line == 2)
		{
			...
			if ( point <= 0)
			{
				printf("HIGH SYMMETRY POINTS IN NEWKPATH_line_2 IS WRONG!\n");
				return kline;
			}
			else
				continue;
	...
	//csv
	vector<string> newLabel = label;
    for (auto &str : newLabel)
        if (str == "GAMMA")
            str = "G";
	FILE* fp1_csv = fopen("KLABELS.csv", "w");
	fprintf(fp1_csv, "K-Label,Coordinate of high-symmetry k-point in band-structure plots\n");
	fprintf(fp1_csv, "%lf,%s\n", kline[0], newLabel[0].c_str());
	int cnt3 = 0;
	for (int i = 1; i < this->num_kpts / point; i++)
	{
		if (high_symmetry[2 * i - 1][0] == high_symmetry[2 * i][0] && high_symmetry[2 * i - 1][1] == high_symmetry[2 * i][1] && high_symmetry[2 * i - 1][2] == high_symmetry[2 * i][2])
			fprintf(fp1_csv, "%lf,%s\n", kline[i * point], newLabel[2 * i].c_str());
		else
			fprintf(fp1_csv, "%lf,%s\n", kline[i * point], (newLabel[2 * i - 1] + '|' + newLabel[2 * i]).c_str());
	}
	fprintf(fp1_csv, "%lf,%s\n", kline[kline.size() - 1], newLabel[newLabel.size() - 1].c_str());
	fclose(fp1_csv);
	printf("Written KLABELS.csv file!\n");
	return kline;
}
...
vector<double> BAND::hse_getkpointsline(int nkpts, vector<vector<double> > kcoor, double vec[3][3])
{
	...
	for (int i = 1; i < point.size(); i++)
	{
		cnt3 += point[i - 1];
		if (high_symmetry[2 * i - 1][0] == high_symmetry[2 * i][0] && high_symmetry[2 * i - 1][1] == high_symmetry[2 * i][1] && high_symmetry[2 * i - 1][2] == high_symmetry[2 * i][2])
			fprintf(fp1_csv, "%lf,%s\n", kline[cnt3], newLabel[2 * i].c_str());
		else
			fprintf(fp1_csv, "%lf,%s\n", kline[cnt3], (newLabel[2 * i - 1] + '|' + newLabel[2 * i]).c_str());
	}
	fprintf(fp1_csv, "%lf,%s\n", kline[kline.size() - 1], newLabel[newLabel.size() - 1].c_str());
	fclose(fp1_csv);
	printf("Written KLABELS.csv file!\n");
...
void BAND::bandcsv(int ISPIN, vector<double> kline, double efermi)
{
	if (ISPIN == 1)
	{
		FILE* fpcsv = fopen("BAND_REFORMATTED.csv", "w");
		// normal
		fprintf(fpcsv, "K-Path,");
		for (int i = 0; i < this->num_bands; i++)
		{
			fprintf(fpcsv, "Band-%d", i+1 );
			if (i < this->num_bands - 1)
                fprintf(fpcsv, ",");
		}
		fprintf(fpcsv, "\n");
		for (int i = 0; i < this->num_kpts; i++)
		{
			fprintf(fpcsv, "%lf,", kline[i - this->start_point]);
			for (int j = 0; j < this->num_bands; j++)
			{
				fprintf(fpcsv, "%lf", this->energy[i][j] - efermi);
				if (j < this->num_bands - 1)
                	fprintf(fpcsv, ",");
			}
			fprintf(fpcsv, "\n");
		}
		fclose(fpcsv);
		printf("Written BAND_REFORMATTED.csv file!\n");
	}
	else if (ISPIN == 2)
	{
		FILE* fpcsv_up = fopen("BAND_REFORMATTED.csv", "w");
		fprintf(fpcsv_up, "K-Path,");
		for (int i = 0; i < this->num_bands; i++)
		{
			fprintf(fpcsv_up, "Band-%d-up,", i+1 );
		}
		for (int i = 0; i < this->num_bands; i++)
		{
			fprintf(fpcsv_up, "Band-%d-dw", i+1 );
			if (i < this->num_bands - 1)
                fprintf(fpcsv_up, ",");
		}
		fprintf(fpcsv_up, "\n");
		for (int i = 0; i < this->num_kpts; i++)
		{
			fprintf(fpcsv_up, "%lf,", kline[i - this->start_point]);
			for (int j = 0; j < this->num_bands; j++)
			{
				fprintf(fpcsv_up, "%lf,", this->energy_up[i][j] - efermi);	
			}
			for (int j = 0; j < this->num_bands; j++)
			{
				fprintf(fpcsv_up, "%lf", this->energy_dw[i][j] - efermi);
				if (j < this->num_bands - 1)
                	fprintf(fpcsv_up, ",");			
			}
			fprintf(fpcsv_up, "\n");	
...
void BAND::bandindex(int ISPIN, vector<double> kline, double efermi)
{
	FILE* fp_index = fopen("BANDKPTS_INFO", "w");
	if (ISPIN == 1)
	{
		fprintf(fp_index, "# 5 energy bands near the Fermi level. \n\n");
		vector<double> near_fermi; //bandindex <energy near fermi>
		for (int i = 0; i < this->num_bands; i++)
		{
			double near_energy;
			if (i % 2 == 0)
			{
				near_energy = abs(this -> energy[start_point][i] - efermi);
				for (int j = start_point; j < this->num_kpts; j++)
				{
					if( abs( this->energy[j][i] - efermi ) < near_energy)
						near_energy = abs(this->energy[j][i] - efermi);
				}
			}
			else
			{
				near_energy = abs(this -> energy[this->num_kpts - 1][i] - efermi);
				for (int j = this->num_kpts - 1; j >= start_point; j--)
				{
					if( abs( this->energy[j][i] - efermi ) < near_energy)
						near_energy = abs(this->energy[j][i] - efermi);
			...
			near_fermi.push_back(near_energy);
		}
		vector<double> sort_elements = near_fermi;
		sort(sort_elements.begin(), sort_elements.end());
		vector<int> element_indices(sort_elements.size(), -1); // To store the indices
		std::unordered_set<int> used_indices; // To track the indices that have been used
		for (int i = 0; i < sort_elements.size(); i++) 
		{
			for (int j = 0; j < near_fermi.size(); j++) 
			{
				if (sort_elements[i] == near_fermi[j] && used_indices.find(j) == used_indices.end()) 
				{
					element_indices[i] = j;
		...
		printf("# 5 energy bands near the Fermi level are \n");
		for (int index = 0; index < 5; index++)
		{
			fprintf(fp_index, "# Band-Index   %d\n", element_indices[index] + 1);
			printf("# Band-Index   %d\n", element_indices[index] + 1);
			if (element_indices[index] % 2 == 0)
			{
				for (int j = start_point; j < this->num_kpts; j++)
				{
					fprintf(fp_index, " %lf	%lf\n", kline[j - this->start_point], this->energy[j][element_indices[index]] - efermi);
				}
			}
			else
			{
	...
	{
		fprintf(fp_index, "# 5 energy bands (spin-up && spin-down) near the Fermi level. \n\n");
		vector<double> near_fermi_up; //bandindex <energy near fermi>
		vector<double> near_fermi_dw; //bandindex <energy near fermi>
		for (int i = 0; i < this->num_bands; i++)
		{
			double near_energy_up;
			double near_energy_dw;
			if (i % 2 == 0)
			{
				near_energy_up = abs(this -> energy_up[start_point][i] - efermi);
				near_energy_dw = abs(this -> energy_dw[start_point][i] - efermi);
				for (int j = start_point; j < this->num_kpts; j++)
				{
					if( abs( this->energy_up[j][i] - efermi ) < near_energy_up)
						near_energy_up = abs(this->energy_up[j][i] - efermi);
					if( abs( this->energy_dw[j][i] - efermi ) < near_energy_dw)
						near_energy_dw = abs(this->energy_dw[j][i] - efermi);
				}
			}
			else
			{
				near_energy_up = abs(this -> energy_up[this->num_kpts - 1][i] - efermi);
				near_energy_dw = abs(this -> energy_dw[this->num_kpts - 1][i] - efermi);
				for (int j = this->num_kpts - 1; j >= start_point; j--)
				{
					if( abs( this->energy_up[j][i] - efermi ) < near_energy_up)
						near_energy_up = abs(this->energy_up[j][i] - efermi);
			...
			near_fermi_up.push_back(near_energy_up);
			near_fermi_dw.push_back(near_energy_dw);
		}
		vector<double> sort_elements_up = near_fermi_up;
		sort(sort_elements_up.begin(), sort_elements_up.end());
		vector<int> element_indices_up(sort_elements_up.size(), -1); // To store the indices
		std::unordered_set<int> used_indices_up; // To track the indices that have been used
		vector<double> sort_elements_dw = near_fermi_dw;
		sort(sort_elements_dw.begin(), sort_elements_dw.end());
		vector<int> element_indices_dw(sort_elements_dw.size(), -1); // To store the indices
		std::unordered_set<int> used_indices_dw; // To track the indices that have been used
		for (int i = 0; i < sort_elements_up.size(); i++) 
		{
			for (int j = 0; j < near_fermi_up.size(); j++) 
			{
				if (sort_elements_up[i] == near_fermi_up[j] && used_indices_up.find(j) == used_indices_up.end()) 
				{
					element_indices_up[i] = j;
					used_indices_up.insert(j);
					break;
				}
			}
			for (int j = 0; j < near_fermi_dw.size(); j++) 
			{
				if (sort_elements_dw[i] == near_fermi_dw[j] && used_indices_dw.find(j) == used_indices_dw.end()) 
				{
					element_indices_dw[i] = j;
					used_indices_dw.insert(j);
					break;
				}
			}
		}
		printf("# 5 energy bands (spin-up && spin-down) near the Fermi level are \n");
		for (int index = 0; index < 5; index++)
		{
			fprintf(fp_index, "# Band-Index of spin-up energy  %d\n", element_indices_up[index] + 1);
			printf("# Band-Index of spin-up energy  %d\n", element_indices_up[index] + 1);
			if (element_indices_up[index] % 2 == 0)
			{
				for (int j = start_point; j < this->num_kpts; j++)
				{
					fprintf(fp_index, " %lf	%lf\n", kline[j - this->start_point], this->energy_up[j][element_indices_up[index]] - efermi);
				}
			}
			else
			{
		...
		for (int index = 0; index < 5; index++)
		{
			fprintf(fp_index, "# Band-Index of spin-up energy  %d\n", element_indices_dw[index] + 1);
			printf("# Band-Index of spin-down energy  %d\n", element_indices_dw[index] + 1);
			if (element_indices_dw[index] % 2 == 0)
			{
				for (int j = start_point; j < this->num_kpts; j++)
				{
					fprintf(fp_index, " %lf	%lf\n", kline[j - this->start_point], this->energy_dw[j][element_indices_dw[index]] - efermi);
				}
			}
			else
			{
	...
	FILE* fkpt = fopen("KPOINTS", "r");
	char buf_kpt[1024];
	while (fgets(buf_kpt, 1024, fkpt) != NULL)
		fputs(buf_kpt, fp_index);
	fclose(fkpt);
	FILE* fkla = fopen("KLABELS", "r");
	char buf_kla[1024];
	while (fgets(buf_kla, 1024, fkla) != NULL)
		fputs(buf_kla, fp_index);
	fclose(fkla);
	fclose(fp_index);
	printf("Written BANDKPTS_INFO file!\n");
}
...
vector<vector<int> > BAND::BandGap(int ISPIN, vector<double> kline, double efermi, bool write)
{
	...
	if (ISPIN == 1)
	{
		...
		vector<int> v, c;
		double V_location[3] = { NULL }, C_location[3] = { NULL };
		int lumo = 0, homo = 0;
	...
	else
	{
		...
		double V_location_up[3] = { NULL }, C_location_up[3] = { NULL }, V_location_dw[3] = { NULL }, C_location_dw[3] = { NULL };
		...
		else
		{
			...
			double V_location_t[3] = { NULL }, C_location_t[3] = { NULL };
			if (CBM_up > CBM_dw)
			{
				CBM_t = CBM_dw;
				lumo_t = lumo_dw;
...
void BAND::getband(int argc, char* argv[])
{
	...
	if (!strcmp(argv[2], "-b") || !strcmp(argv[2], "-B") || !strcmp(argv[2], "-hb") || !strcmp(argv[2], "-HB"))
	{
		...
		bandcsv(atoi(ISPIN.c_str()), kline, efermi);
	}
	if (!strcmp(argv[2], "-id") || !strcmp(argv[2], "-index"))
	{
		bandindex(atoi(ISPIN.c_str()), kline, efermi);
	}
	...
	if (!strcmp(argv[2], "-se") || !strcmp(argv[2], "-SE") || !strcmp(argv[2], "-hse") || !strcmp(argv[2], "-HSE"))	// output select elements
	{
		vector<INPUT> data;
		for (int i = 3; i < argc; i++)
		{
	...
	if (!strcmp("-em", argv[2]) || !strcmp("-hem", argv[2]))	//effective mass
	{
		...
		for (int i = 0; i < pd.size(); i++)
		{
			...
			{
				if (kp[j].label == pd[i][0] || (kp[j].label == "GAMMA" && pd[i][0] == "G"))
				{
