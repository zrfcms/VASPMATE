...
extern vector<Fl_Color> sv_color = { FL_RED, FL_BLUE,FL_GREEN ,FL_YELLOW, FL_MAGENTA, FL_DARK_RED,FL_DARK_BLUE, FL_DARK_GREEN,FL_DARK_YELLOW,FL_DARK_MAGENTA };
...
void addbenchmarkX(Fl_Chart_SVMOD* chart)
{
	...
	chart->add(y.size(), x, y, "MarkX", FL_BLACK, FL_DASH, 2, FL_NO_SYMBOL_SVMOD, 3);
...
void addbenchmarkY(Fl_Chart_SVMOD* chart)
{
	...
	vector<double> x{ 0,0 };
	chart->add(y.size(), x, y, "MarkY", FL_BLACK, FL_DASH, 2, FL_NO_SYMBOL_SVMOD, 3);
	chart->set_name_flag(chart->numb - 1, 0);
}

double get_Enthalpy_formation(const char file1[] , const char file2[] , vector<double> en)
{
	double energy = 0;
	FILE* fp_ = fopen(file2, "r");
	if (fp_ == NULL)
	{
...
void Plotcurve(vector<double> X, vector<double> Y, string x_label, string y_label, string figname, string filename)
{
	int color_cnt = 0;
	...
	double space_x = (xmax - xmin) / 8;
	double space_y = (ymax - ymin) / 5;
	...
	chart->add(X.size(), X, Y, figname.c_str(), sv_color[color_cnt], FL_SOLID, 2, FL_CIRCLE_SVMOD, 3);
	chart->write(filename.c_str());
	delete chart;
}

void Plotcurve(vector<int> X, vector<double> Y, string x_label, string y_label, string figname, string filename)
{
	int color_cnt = 0;
	double xmin = *min_element(X.begin(), X.end());
	double ymin = *min_element(Y.begin(), Y.end());
...
void moledynamic::poltPCDAT(const char file[])
{
	FILE* fp = fopen(file, "r");
	if (fp == nullptr)
	{
	...
	fclose(fp);
	int nsw = atoi(GetInfoINCAR("NSW").c_str());
	int potim = atoi(GetInfoINCAR("POTIM").c_str());
	string figname = "PCDAT." + to_string(nsw * potim);
	Plotcurve(r_ang, pcf, "r(Ang)", "PCF", figname, "PCDAT.spco");
	printf("Written PCDAT.spco file!\n");
}

void moledynamic::poltEnergy(const char file[])
{
	FILE* fp = fopen(file, "r");
	if (fp == nullptr)
	{
	...
	fclose(fp);
	auto myplotcurve = [](vector<double> X, vector<double> Y, string x_label, string y_label, string figname, string filename) {
		int color_cnt = 0;
		double xmin = *min_element(X.begin(), X.end());
		double ymin = *min_element(Y.begin(), Y.end());
		double xmax = *max_element(X.begin(), X.end());
		double ymax = *max_element(Y.begin(), Y.end());
		bound_fix(xmin, xmax, ymin, ymax);
		double space_x = (double)((int)((xmax - xmin) / 8));
		double space_y = (ymax - ymin) / 5;
		Fl_Chart_SVMOD* chart = new Fl_Chart_SVMOD();
		chart->titles(x_label.c_str(), y_label.c_str());
		chart->space(space_x, space_y);
		chart->bound(xmin, ymin, xmax, ymax);
		chart->add(X.size(), X, Y, figname.c_str(), sv_color[color_cnt], FL_SOLID, 2, FL_NO_SYMBOL_SVMOD, 3);
		chart->write(filename.c_str());
		delete chart;
	};
	myplotcurve(step, en, "N_step", "Total_energy(eV/cell)", "energy.dat", "Energy.spco");
	printf("Written Energy.spco file!\n");
}