...
int checkTorF(char* arr, size_t size) 
{
	...
	const int types[])
{
		...
		dataset->n_operations))
	{
		...
		if (centering == PRIMITIVE)
			...
			pearson.push_back('R');
		pearson += to_string(num_atom);
		fprintf(fp, "Pearson symbol: %s\n", pearson.c_str());
		fclose(fp);
		if (spgtype.number) {
...
void EV_get_symmetry(const char file1[])
{
	...
	fprintf(fp_, "****************************************************\n");
	fclose(fp_);
	Output_to_terminal("SYMMCAR");
	printf("Written SYMMCAR file!\n");
	free(types);
}
...
void EV_affine(const char file1[], const char file2[], char model[], char mode[], double init_value, double step_length, int step_num)
{
	FILE* fp = fopen("structure_operator", "at+");
	fprintf(fp, "****************************************************\n");
	...
	for (int i = 0; i < step_num + 1; i++)
	{
		...
		sprintf(file_name[i], "%s_%.4lf", file2, current_strain);
		...
		fprintf(fp_, "****************************************************\n");
		fclose(fp_);
		if(init_value == step_length)
			copy_file(file2, file_name[i]);
	}
	free(types);
	delete[] file_name;
}

void EV_alias_tensile(const char file1[], const char file2[], char mode[], double istart, double iend, double ispacing, double value)
{
	FILE* fp = fopen("structure_operator", "at+");
	fprintf(fp, "****************************************************\n");
	fprintf(fp, "FILE NAME: %s\n", file1);
	...
	else
	{
		...
		fprintf(fp_error, "****************************************************\n");
		fclose(fp_error);
		printf("ERROR!The %s is a wrong parameter!\n", mode);
		return;
	}
	int step_num;
	if (ispacing == 0)
	...
	for (int i = 0; i < step_num; i++)
	{
		...
		sprintf(file_name[i], "%s_%.4lf", file2, current);
		fprintf(fp, "FILE NAME: %s\n", file_name[i]);
		fclose(fp);
...
void EV_alias_shear(const char file1[], const char file2[], char mode[], double istart1, double iend1, double ispacing1,
	double istart2, double iend2, double ispacing2, double value)
{
	...
	for (int i = 0; i < max(step_num1, step_num2); i++)
	{
		...
		sprintf(file_name[i], "%s_%.4lf_%.4lf", file2, current1, current2);
		fprintf(fp, "FILE NAME: %s\n", file_name[i]);
		fclose(fp);
		FILE* fp_ali = fopen(file_name[i], "w");
		if (!strcmp("xy", mode))
		...
		printf("Written %s file!\n", file_name[i]);
		if(istart1 == iend1 && istart2 == iend2)
			copy_file(file2, file_name[i]);
	}
	free(types);
	delete[] file_name;
...
void EV_redefine(const char file1[], const char file2[], int rot[3][3], double symprec, int a1, int x1, int a2, int x2,bool fixdirect)
{
	...
	EV_supercell(file1, "temp_super", super);
	FILE* _fp = fopen("structure_operator", "at+");
	...
	FILE* fp_super = fopen("temp_super", "r");
...
void EV_fixatomcoor(const char file1[], const char file2[], char axis[], double start, double end, char addfix[3])
{
	...
	int result = checkTorF(addfix, 3);
	if (result != 0)
	{
		printf("Detection shows that [fix-tag]s are neither F nor T; all will be fixed using T T T !\n");
		addfix[0] = 'T'; addfix[1] = 'T'; addfix[2] = 'T';
	}
	readposcar(fp1, title, latt, ifix, iflg, nant, typenum, elemnum, elemsym, vec, xyz, fix);
	fclose(fp1);
	direct_to_carts(iflg, vec, xyz, nant);
...
void EV_fixatomele(const char file1[], const char file2[], vector<string> elem, char addfix[][3])
{
	...
	for (int i = 0; i < nant[1]; i++)
	{
		...
		if (find(elem.begin(), elem.end(), elemsym[i]) != elem.end())
		{
			...
			{
				for (int j = 0; j < 3; j++)
					fix[k + cnt][j] = addfix[fix_numb][j];
				int result = checkTorF(fix[k + cnt], 3);
				if (result != 0)
				{
					isfix = false;
					fix[k + cnt][0] = 'T'; fix[k + cnt][1] = 'T'; fix[k + cnt][2] = 'T';
				}		
			}
		fix_numb++;
		}
		if (!isfix)
			printf("Detection shows that [fix-tag]s of %s are neither F nor T; all will be fixed using T T T !\n", elemsym[i]);
		cnt += typenum[i];
	}
	FILE* fp2 = fopen("structure_operator", "at+");
...
void EV_cleanfix(const char file[])
{
	...
	fclose(fp2);
	printf("Fix has been canceled!\n");
}

void EV_swapele(const char file1[], const char file2[], vector<string> elem)
{
	FILE* fp1 = fopen(file1, "r");
	if (fp1 == NULL)
	{
	...
	readposcar(fp1, title, latt, ifix, iflg, nant, typenum, elemnum, elemsym, vec, xyz, fix); //default elem
	fclose(fp1);
	int minnumb = 0;
	if (elem.size() < nant[1])
	{
		printf("The number of elements is fewer than the number in the %s file.\n", file1);
		printf("VASPMATE will be replaced one by one in the order of the elements provided.\n");
		minnumb = elem.size();
	}
	else if (elem.size() > nant[1])
	{
		printf("The number of elements is greater than the number in the %s file.\n", file1);
		printf("VASPMATE will be replaced one by one in the order of elements in the file.\n");
		minnumb = nant[1];		
	}
	else
	{
	...
	for (int i = 0; i < minnumb; i++)
	{
		strcpy(elemsym[i], elem[i].c_str());
	}
	strcpy(title, "Swap elements using VASPMATE.");
	FILE* fp3 = fopen(file2, "w");
	savposcar(fp3, title, latt, ifix, iflg, nant, typenum, elemnum, elemsym, vec, xyz, fix);
	printf("Written %s file!\n", file2);
}
...
void EV_atomsort_coord(const char file1[], const char file2[], char mode[])
{
	...
	readposcar(fp1, title, latt, ifix, iflg, nant, typenum, elemnum, elemsym, vec, xyz, fix);
	fclose(fp1);
	// This was removed as requested by the boss. If any bugs related to this section arise later, please revert to the original.
	// direct_to_carts(iflg, vec, xyz, nant);
	int axis;
	if (!strcmp(mode, "a"))
		axis = 0;
...
void EV_move(const char file1[], const char file2[], char label[], char axis[], double move[3], double mmin, double mmax)
{
	...
	if (!strcmp("a", axis))
		ax = 0;
	else if (!strcmp("b", axis))
		ax = 1;
	else if (!strcmp("c", axis))
		ax = 2;
	else
