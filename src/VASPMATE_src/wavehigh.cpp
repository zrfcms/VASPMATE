#include "../../include/VASPMATE_include/wavehigh.h"
...
namespace ionizing {
  ...
  /*************************************************************************/
	...
	const MatrixX3i WAVEHIGH::getGVectors(const int ikpoint, bool check_consistency) const {
		if (ikpoint >= getHeader()._nKpoints) {
			std::cerr << "******** Invalid ikpoint index, please check. ********" << std::endl;
			std::abort();
		}
		return gen_gvectors(_nGrid, ikpoint, false, check_consistency);
	}
	...
	void WAVEHIGH::save_as_vesta(const Cubcd& phi,
		...
		if (0 == mode) {
			sprintf(out_file_name, "%s%s", prefix, "_IMAG.vasp");
		}
		else {
			sprintf(out_file_name, "%s%s", prefix, "_REAL.vasp");
		}
			...
			for (int k = 0; k != nz; ++k) {
				for (int j = 0; j != ny; ++j) {
					for (int i = 0; i != nx; ++i) {
						...
						if (nwrite % 10 == 0 || nwrite == nx * ny * nz) {
							strcat(line, "\n");
							out << line;
							strcpy(line, "");
						}
		...
		out.close();
		Chgcarinterpolation(out_file_name, out_file_name);
		return;
	}
	...
	void WAVEHIGH::plotWave(const int ispin,
		...
		const MatrixX3i gvecs = getGVectors(ikpoint, true);
		...
		using std::endl;
		std::stringstream ss;
		std::string ispin_title;
		if (getInfo()._nSpin == 2)
			ispin_title = ispin == 0 ? "UP_" : "DW_";
		else
			ispin_title = "";
		ss << prefix << "_" << ispin_title << "B" << setfill('0') << setw(3) << iband + 1
			<< "_K" << setfill('0') << setw(3) << ikpoint + 1;
		saveAsVesta(phi, poscar, ss.str().c_str(), is_real);
		
		cout << "plotWave at spin " << setw(2) << ispin
			<< " kpoint " << setw(2) << ikpoint + 1
			<< " band " << setw(2) << iband + 1 << " finished." << endl;
	}
