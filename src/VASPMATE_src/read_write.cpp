...
/* I always feel that it is very inconvenient to get the atomic symbol of each atom right now. */
	int numb_hzy = 0;
	for (int i = 0; i < poscar.nant[1]; i++)
	{
		for (int j = 0; j < poscar.typenum[i]; j++)
		{
			strcpy(poscar.elemeach[numb_hzy + j], poscar.elemsym[i]);
		}		
		numb_hzy = numb_hzy + poscar.typenum[i];
	}
	return 0;
}

int readxdatcar(FILE* fp, POSCAR& poscar)
{
	//	printf(" >>> Reading ASCII file\n");
	int    iline;
	int    flag;
